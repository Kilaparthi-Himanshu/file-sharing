...
import joinSession from '@/app/functions/session/joinSession';
...
export default function JoinSessionModal({removeModal} : {removeModal: () => void}) {
    ...
    const { mutateAsync: sessionJoin, isPending } = useMutation({
        mutationFn: async ({ 
            displayName,  
            sessionId, 
            password
            ...
            sessionId: string, 
            password: string
        }) => await joinSession({ displayName, sessionId, password })
    });
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const data = await sessionJoin({ displayName, sessionId, password });
            ...
            >
                ...
                >
                    ...
                    <form onSubmit={handleSubmit} className='w-full h-full flex flex-col items-center justify-center'>
                        <span className='text-4xl font-light'>Join Session</span>
